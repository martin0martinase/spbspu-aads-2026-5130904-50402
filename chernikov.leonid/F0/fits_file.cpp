    ...
    try {
        switch (bitpix) {
            ...
            case 16: {
                ...
                for (auto& v : data_int16) swapBytes(v);
                break;
            }
            case 32: {
                ...
                for (auto& v : data_int32) swapBytes(v);
                break;
            }
            case -32: {
                ...
                for (auto& v : data_float32) swapBytes(v);
                break;
            }
            case -64: {
                ...
                for (auto& v : data_float64) swapBytes(v);
                break;
            }
            default:
...
void FitsFile::writeHeader(std::ostream& out) const {
    ...
    for (const auto& [key, value] : headerVec) {
        ...
        if (!valStr.empty()) {
            ...
            for (size_t i = 0; i < valStr.length(); i++) {
                if (i == 0 && (valStr[i] == '-' || valStr[i] == '+')) continue;
                if (valStr[i] == '.') { hasDot = true; continue; }
                if (valStr[i] >= '0' && valStr[i] <= '9') continue;
                if (valStr == "TRUE" || valStr == "FALSE") { isNum = true; break; }
                isNum = false;
                break;
            }
    ...
    endLine[0] = 'E'; endLine[1] = 'N'; endLine[2] = 'D';
    headerStr += endLine;
...
double FitsFile::getPixelAsDouble(int x, int y) const {
    ...
    switch (dataType) {
        case PixelType::UINT8:   return static_cast<double>(data_uint8[idx]);
        case PixelType::INT16:   return static_cast<double>(data_int16[idx]);
        case PixelType::INT32:   return static_cast<double>(data_int32[idx]);
        case PixelType::FLOAT32: return static_cast<double>(data_float32[idx]);
        case PixelType::FLOAT64: return data_float64[idx];
        default: return 0.0;
    }
}
...
void FitsFile::setPixelAsDouble(int x, int y, double value) {
    ...
    switch (dataType) {
        case PixelType::UINT8:   data_uint8[idx]  = static_cast<uint8_t>(std::max(0.0, std::min(255.0, value))); break;
        case PixelType::INT16:   data_int16[idx]  = static_cast<int16_t>(std::max(-32768.0, std::min(32767.0, value))); break;
        case PixelType::INT32:   data_int32[idx]  = static_cast<int32_t>(value); break;
        case PixelType::FLOAT32: data_float32[idx] = static_cast<float>(value); break;
        case PixelType::FLOAT64: data_float64[idx] = value; break;
        default: break;
    }
}
...
bool FitsFile::calibrate(const FitsFile& darkFrame) {
    ...
    if (width != darkFrame.width || height != darkFrame.height) {
        std::cerr << "Error: Image and dark frame dimensions don't match ("
                  << width << "x" << height << " vs "
    ...
    for (int y = 0; y < height; y++) {
        for (int x = 0; x < width; x++) {
            ...
            double calibrated = original - dark;
            setPixelAsDouble(x, y, calibrated);
        }
    }
...
void FitsFile::printInfo() const {
    ...
    std::cout << "  Data type:       ";
    switch (bitpix) {
        case 8:   std::cout << "unsigned 8-bit integer"; break;
        case 16:  std::cout << "16-bit signed integer"; break;
        case 32:  std::cout << "32-bit signed integer"; break;
        case -32: std::cout << "32-bit floating point"; break;
        case -64: std::cout << "64-bit floating point"; break;
