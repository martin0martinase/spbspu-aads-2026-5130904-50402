...
namespace chernikov
{
  ...
  BOOST_AUTO_TEST_CASE(GetPriorityUnaryOperators)
  {
    BOOST_CHECK_EQUAL(detail::getPriority('!'), 3);
    BOOST_CHECK_EQUAL(detail::getPriority('~'), 3);
  }
  ...
  BOOST_AUTO_TEST_CASE(LogicalNotOperator)
  {
    std::string expr = "! 0";
    Queue<std::string> postfix = detail::infixToPostfix(expr);
    std::vector<std::string> result = queueToVector(postfix);
  ...
  BOOST_AUTO_TEST_CASE(BitwiseNotOperator)
  {
    std::string expr = "~ 5";
    Queue<std::string> postfix = detail::infixToPostfix(expr);
    std::vector<std::string> result = queueToVector(postfix);
  ...
  BOOST_AUTO_TEST_CASE(UnaryOperatorPrecedence)
  {
    std::string expr = "! 0 + 5";
    Queue<std::string> postfix = detail::infixToPostfix(expr);
    std::vector<std::string> result = queueToVector(postfix);
  ...
  BOOST_AUTO_TEST_SUITE(EvaluateExpressionTests)

  BOOST_AUTO_TEST_CASE(EvaluateLogicalNotZero)
  {
    long long result = chernikov::evaluateExpression("! 0");
    BOOST_CHECK_EQUAL(result, 1);
  }

  BOOST_AUTO_TEST_CASE(EvaluateLogicalNotNonZero)
  {
    long long result = chernikov::evaluateExpression("! 42");
    BOOST_CHECK_EQUAL(result, 0);
  }

  BOOST_AUTO_TEST_CASE(EvaluateLogicalNotNegative)
  {
    long long result = chernikov::evaluateExpression("! -5");
    BOOST_CHECK_EQUAL(result, 0);
  }

  BOOST_AUTO_TEST_CASE(EvaluateBitwiseNot)
  {
    long long result = chernikov::evaluateExpression("~ 0");
    BOOST_CHECK_EQUAL(result, -1);
  }

  BOOST_AUTO_TEST_CASE(EvaluateBitwiseNotPositive)
  {
    long long result = chernikov::evaluateExpression("~ 5");
